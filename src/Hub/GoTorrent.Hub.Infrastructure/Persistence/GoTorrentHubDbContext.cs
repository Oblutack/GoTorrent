...
/// engine node — RSS rules, and (once built) history/analytics, Identity.
/// SQLite for dev (see DependencyInjection.AddRssRules); nothing here
/// uses a SQLite-specific type mapping, so swapping to Npgsql later is a
/// provider change, not a schema rewrite.
/// </summary>
public sealed class GoTorrentHubDbContext(DbContextOptions<GoTorrentHubDbContext> options) : DbContext(options)
{
